...
void Parent::update(){
	...
	switch ( mNextSequence ){
		case NEXT_TITLE:
			STRONG_ASSERT( !mTitle && ( mGame || mGameOver || mEnding ) ); //チェック
			SAFE_DELETE( mGame );
			SAFE_DELETE( mGameOver );
			SAFE_DELETE( mEnding );
			mTitle = new Title();
			break;
		case NEXT_GAME:
			STRONG_ASSERT( mTitle && !mGameOver && !mGame && !mEnding ); //チェック
			SAFE_DELETE( mTitle );
			mGame = new Game::Parent( mMode );
			break;
		case NEXT_GAME_OVER:
			STRONG_ASSERT( mGame && !mGameOver && !mEnding && !mTitle ); //チェック
			SAFE_DELETE( mGame );
...
void Parent::moveTo( NextSequence next ){
	STRONG_ASSERT( mNextSequence == NEXT_NONE ); //これ以外ありえない
	mNextSequence = next;
}
