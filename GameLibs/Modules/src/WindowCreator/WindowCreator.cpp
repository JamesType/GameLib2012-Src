...
string TCHARArrayToString(const TCHAR* pSrc)
{
#ifdef UNICODE
	string result(wcslen(pSrc) * 2 + 1, '\0');
	size_t len = WideCharToMultiByte(CP_UTF8, 0, pSrc, -1, &result[0], result.size(), NULL, NULL);
	result.erase(len);
	return result;
#else
	return string(pSrc);
...
int APIENTRY _tWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPTSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	//ウィンドウ状態保持クラス生成
	gImpl = NEW Impl();
	gImpl->mCommandLine = TCHARArrayToString(lpCmdLine);
	WindowCreator wc;
	//ユーザ設定関数呼び出し
...
#ifdef UNICODE
	MultiByteToWideChar(CP_UTF8, 0, wc.title(), wc.titleLength(), szTitle, MAX_LOADSTRING);
	memcpy(szWindowClass, L"GameLibWindowCreator", sizeof(L"GameLibWindowCreator"));
#else
