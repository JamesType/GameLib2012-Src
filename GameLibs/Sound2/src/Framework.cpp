...
//2Dレイヤー
	...
	void preUpdate(){
		...
		pointerScale *= 0.5f; //画面解像度が半分なので修正
...
//サンプルライブラリ用
	void playSound( const short* data, int size ){
		mSoundWave = Sound::Wave::create( data, size * 2, 44100, 1 );
		mSoundPlayer = Sound::Player::create( mSoundWave );
		mSoundPlayer.play();
	}
	Array< unsigned > mVideoMemoryWithPadding;
	Graphics::Texture m2dTexture;
	static const unsigned MAGIC_NUMBER = 0x12345678;
	Sound::Wave mSoundWave;
	Sound::Player mSoundPlayer;
...
void Framework::playSound( const short* data, int size ){
	gImpl->playSound( data, size );
}
