...
mEnergy( mMaxEnergy ),
mLockOn( true ){ //向き合って始まaaaaaaaaaるので最初はロックオンしてていいだろう。
	mDatabase = new GraphicsDatabase( "robo.txt" );
	mTree = mDatabase->createTree( "robo" );
	mTree->setAnimation( mDatabase->animation( "standing" ) ); //立ちアニメセット
...
Robo::~Robo(){
	...
	SAFE_DELETE( mTree ); //使っている方が使われている方より先
	...
	switch ( mMode ){
		...
		case MODE_WALKING:
			if ( iJump ){
				...
				mTree->setAnimation( mDatabase->animation( "jumping" ) ); //ジャンプ
			}else if ( iTurn ){
...
void Robo::move( bool left, bool right, bool up, bool down ){
	...
	if ( move.x != 0.0 || move.z != 0.0 ){
		if ( mMode == MODE_STANDING ){
			mMode = MODE_WALKING;
			mTree->setAnimation( mDatabase->animation( "walking" ) ); //歩き出した
		}
	}else{
		if ( mMode == MODE_WALKING ){
			mMode = MODE_STANDING;
			mTree->setAnimation( mDatabase->animation( "standing" ) ); //静止
		}
	}
