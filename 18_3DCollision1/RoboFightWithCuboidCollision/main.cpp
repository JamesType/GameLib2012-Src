...
namespace GameLib{
	void Framework::update(){
		...
		//衝突検出用Cuboidゲット
		const Cuboid& cbRobo0 = *gRobo[ 0 ]->cuboid();
		const Cuboid& cbRobo1 = *gRobo[ 1 ]->cuboid();
		const Cuboid& cbStage = *gStage->cuboid();
		//移動方向を制限
		//0番ロボ
		cbRobo0.restrictMove( &move[ 0 ], cbRobo1 );
		cbRobo0.restrictMove( &move[ 0 ], cbStage );
		//1番ロボ
		cbRobo1.restrictMove( &move[ 1 ], cbRobo0 );
		cbRobo1.restrictMove( &move[ 1 ], cbStage );
